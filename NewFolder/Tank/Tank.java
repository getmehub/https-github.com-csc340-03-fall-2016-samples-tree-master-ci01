 ...
 * @author MacBook
 */
public class Tank extends GameObj implements Observer {
    ...
    Tank(Image img, int life, int x, int y, int Yspeed, int up, int down, int left, int right, int fire) {
        ...
        try{
            this.basicBulletStrip = ImageIO.read(Tank.class.getResource("Resources/Shell_basic_strip60.png"));
            this.strongBulletStrip= ImageIO.read(Tank.class.getResource("Resources/Shell_heavy_strip60.png"));
            this.shieldImg = ImageIO.read(Tank.class.getResource("Resources/Shield1.png"));
            this.healthBars[0] = ImageIO.read(Tank.class.getResource("Resources/health.png"));
            this.healthBars[1] = ImageIO.read(Tank.class.getResource("Resources/health1.png"));
            this.healthBars[2] = ImageIO.read(Tank.class.getResource("Resources/health2.png"));
            this.healthBars[3] = ImageIO.read(Tank.class.getResource("Resources/health3.png"));
        }
        catch(Exception e){
    ...
    public void isDied() {
       ...
       tankgame.explosion.add(new TankExplosion(x,y,7,tankgame.bigExp));//explosion.add(new TankExplosion(x,y,7,tankgame.bigExp));
        sp.play();
        boom = true;
    ...
    public void draw(Graphics g, ImageObserver obs) {
        if(!boom){
            ...
            if(this.health >= 150){
                healthbar = new GameObj(healthBars[0],x,y+height,Yspeed);
                healthbar.draw(g, obs);
            }
            if(this.health < 150 && this.health >=100){
                healthbar = new GameObj(healthBars[1],x,y+height,Yspeed);
                healthbar.draw(g, obs);
            }
            if(this.health < 100 && this.health >=50){
                healthbar = new GameObj(healthBars[2],x,y+height,Yspeed);
                healthbar.draw(g, obs);
            }
            if(health < 50){
                healthbar = new GameObj(healthBars[3],x,y+height,Yspeed);
                healthbar.draw(g, obs);
            }
         }
